...
playGame game = do
  imapM_
    (\i u -> putMessage u $ UpdateGameView $ toGameView i game)
    (users game)
  event <-
    runConcurrently $
      asum $
...
update event game = case event of
  UserAction ix action -> case action of
    PlayCard c -> case currentRound game of
      ...
      Playing r ->
        if Set.member c (validCards r ix)
...
toGameView ix g = case currentRound g of
  ...
  Playing r ->
    GameView
      { hand =
          map (\c -> HandCard c (Set.member c (validCards r ix))) $
            toList $
              cards $
                index (players r) ix,
        table =
          (drop <> take) (fromIntegral ix) $
            toList $
